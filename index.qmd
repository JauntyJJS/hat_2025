...
My harmoniation template organisation is based on the R package `rcompendium` but there are others as well.
...
![](images/start_up_enviromental_files.png){fig-alt="Figure showing the .Renviron and .Rprofile file in an R project template." fig-align="center" width="75%"}

## what goes in `.Renviron`

:white_check_mark: R-specific [environment variables](https://cran.r-project.org/doc/manuals/r-devel/R-ints.html){target="_blank"}.\
...
R Packages -- [Structure](https://r-pkgs.org/Structure.html){target="_blank"}
...
## Binary R Packages Installation

Go for ({{< fa brands windows >}} | {{< fa brands apple >}}) binary R packages in CRAN.
...
## Source R Packages Installation

Go for source R packages if you need the latest version urgently.
...
![](images/binary_not_latest.png){fig-alt="Figure showing the Windows binary files are not the latest version for R package parallelly." fig-align="center" width="70%"}
 ...
 {{< fa brands linux >}} only have source R packages in CRAN.
